...
class GridMap:
    ...
    def animate(self, start, goal, calcPath):
        ...
        for i in range(1, len(moves)):
            print([robot[i-1][0] + moves[i][0], robot[i-1][1] + moves[i][1]])
        ...
        for i in range(0, len(robot)):
            dynamisch.append((i, self.dynamic_obstacle(ax, [robot[i][0], robot[i][1]])))
        


        def animate(i):
            time_text.set_text((i))
            if(dynamisch[i][1] != None and dynamisch[i][0] < i):
                ax.add_patch(dynamisch)
