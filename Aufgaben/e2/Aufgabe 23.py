...
def calcPath(robot, obstacles, target, map, steps):
    ...
    PathHistoryBool = [False]
    moves = [(0, 1*steps), (0, -1*steps), (1*steps, 0), (-1*steps, 0)]
    counter = 0
    ...
    while(counter < 10000000):
        for move in moves:
            ...
            if((not collision(temp, obstacles) and not(temp in PathHistory)) and isInMap(temp, map) and PathHistoryBool[i] == False):
                lastMoves = PathHistoryMoves[i].copy()
                lastMoves.append(move)
                PathHistoryMoves.append(lastMoves)
                PathHistory.append(temp)
                PathHistoryBool.append(False)
                PathHistoryBool[i] = True

        for j in range(0, len(PathHistory)):
            print(j)
            time.sleep(0.1)
            if(not PathHistoryBool[j]):
                if(collisionWithTarget(PathHistory[j], target)):
                    return PathHistoryMoves[j]
                if(distanceToTarget(PathHistory[j], target) < distance):
                    distance = distanceToTarget(PathHistory[j], target)
                    i = j
