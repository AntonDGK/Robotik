...
def collision(robot, obstacles):
    ...
    for obstacle in obstacles:
        ...
        maxYob = 0
        for o in obstacle:
            minXob = min(minXob,o[0])
            minYob = min(minYob,o[1])
            maxXob = max(maxXob,o[0])
            maxYob = max(maxYob,o[1])
        if((minXob - maxX <= xSum <= maxXob + maxX) and (minYob - maxY <= ySum <= maxYob + maxY)):
            return True
    return False
...
def isInMap(robot, map):
    ...
    maxX = maxX - xSum
    maxY = maxY - ySum
    if((map[0][0]<= xSum <= map[0][1]) and (map[1][0] <= ySum <= map[1][1])):
        return True
    return False
...
def calcPath(robot, obstacles, target, map, steps):
    ...
    while(not collisionWithTarget(PathHistory[i], target) and i < 10000000):
        ...
        stepsCount = 10000000
        #print(PathHistoryDistance)
        tempDistance = i
        for pathIndex in range(0, len(PathHistory)):
            if(not PathHistoryChecked[pathIndex]):
                if((PathHistoryDistance[pathIndex] < distance)):
                    distance = PathHistoryDistance[pathIndex]
                    i = pathIndex 
                if(len(PathHistoryMoves[pathIndex]) < stepsCount):
                    stepsCount = len(PathHistoryMoves[pathIndex])
                    distenceStepCount = PathHistoryDistance[pathIndex]
                    j = pathIndex
                if(len(PathHistoryMoves[pathIndex]) == stepsCount and (PathHistoryDistance[pathIndex] < distenceStepCount)):
                    distenceStepCount = PathHistoryDistance[pathIndex]
                    j = pathIndex

        if(PathHistoryDistance[tempDistance] < PathHistoryDistance[pathIndex]):
            i = j
        counter = counter + 1
        #print(i)
    print(counter)
...
robot = [(900, 100), (1000, 100), (1000, 200), (900, 200)]
